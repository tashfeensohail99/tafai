...
async function main(): Promise<void> {
  ...
  try {
    ...
    // one — so it must go over DIRECT_URL (5432, session mode) or it fails with
    // 25001. Best-effort: if it can't run, autovacuum reclaims the dead tuples
    // on its own. Plain VACUUM (never FULL) — no exclusive lock.
