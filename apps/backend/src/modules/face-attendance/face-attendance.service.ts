...
import { extractFaceEvent, isCaptureEvent } from './hik-multipart';
...
export class FaceAttendanceService implements OnModuleInit, OnModuleDestroy {
  ...
  // Which NVR event types carry a snapshot worth recognising. Motion is included
  // by default because value-tier cameras never emit face events (see
  // isCaptureEvent); set FACE_CAPTURE_EVENT_TYPES=face to tighten this where the
  // hardware really does do face capture.
  private readonly captureEventTypes = (
    process.env.FACE_CAPTURE_EVENT_TYPES ??
    'face,vmd,motion,linedetection,fielddetection'
  )
    .split(',')
  ...
  async ingestNvrPush(
    ...
    // Split the two rejection reasons apart and LOG them. Folded together and
    // silent, this was the worst possible failure mode: the NVR pushes, we
    // answer 200 OK, and nothing appears anywhere — indistinguishable from the
    // NVR never having been configured at all.
    if (!isCaptureEvent(ev.eventType, this.captureEventTypes)) {
      this.log.debug(
        `ignored push: eventType="${ev.eventType ?? '(none)'}" not in [${this.captureEventTypes.join(', ')}] — widen FACE_CAPTURE_EVENT_TYPES if this camera should count`,
      );
      return { status: 'ignored' };
    }
    if (!ev.image || ev.image.length < 512) {
      this.log.warn(
        `ignored push: ch=${ev.channelId ?? '?'} eventType="${ev.eventType}" carried no usable image (${ev.image?.length ?? 0} bytes) — plain motion alerts are often XML-only; enable Face Detection on this channel, or tick the Capture linkage`,
      );
      return { status: 'ignored' };
    }
  ...
  // ── admin: capture review (read-only) ─────────────────────────────────────
  /**
   * Recent NVR captures, newest first — the "are the cameras actually seeing
   * anyone, and who?" review list behind the attendance console.
   *
   * FaceCaptureEvent has no Prisma relation to Employee (matchedEmployeeId is a
   * bare scalar column, deliberately, so a capture survives an employee delete),
   * so the name comes from a raw LEFT JOIN rather than an N+1 lookup per row.
   */
  async listRecentCaptures(limit?: number, matchedOnly = false) {
    const take = Math.min(200, Math.max(1, Math.trunc(limit ?? 30) || 30));
    ...
    >(
      `SELECT fce.id,
              fce."channelId",
              fce.status::text AS status,
              fce.similarity,
              fce."capturedAt",
              fce."imageObjectKey",
              NULLIF(TRIM(CONCAT_WS(' ', e."firstName", e."lastName")), '') AS "employeeName"
         FROM core.face_capture_events fce
         LEFT JOIN core.employees e ON e.id = fce."matchedEmployeeId"
        ${matchedOnly ? `WHERE fce.status = 'MATCHED'` : ''}
        ORDER BY fce."capturedAt" DESC
        LIMIT $1`,
      take,
    );
    return rows.map((r) => ({
      ...
      status: r.status,
      similarity: r.similarity === null ? null : Math.round(Number(r.similarity) * 1000) / 1000,
      capturedAt: r.capturedAt,
      employeeName: r.employeeName,
      hasImage: Boolean(r.imageObjectKey),
    }));
  }

  /**
   * The stored JPEG for one capture. Served through our own authenticated route
   * (mirrors the WhatsApp media endpoint) because the media bucket is private —
   * we never hand a signed bucket URL to the browser.
   */
  async getCaptureImage(id: string): Promise<{ bytes: Buffer; mimeType: string }> {
    const event = await this.prisma.faceCaptureEvent.findUnique({
      where: { id },
      select: { imageObjectKey: true },
    });
    if (!event) throw new NotFoundException('Capture not found');
    if (!event.imageObjectKey) throw new NotFoundException('This capture has no stored image');
    const { bytes, mimeType } = await this.storage.download(event.imageObjectKey);
