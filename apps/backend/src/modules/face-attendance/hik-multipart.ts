...
/** Is this specifically a face-capture event (vs. motion, heartbeat, test)? */
...
/**
 * Event types that carry a snapshot we can run recognition on.
 *
 * Deliberately WIDER than isFaceEvent. Only mid/high-tier Hikvision cameras and
 * DeepinMind ("iDS-") NVRs emit face events at all — value-tier AcuSense models
 * (e.g. DS-2CD1343G2-LIU) top out at motion-with-human-target and have no Smart
 * Event section whatsoever. Those still push a perfectly good JPEG on VMD, and
 * our own InsightFace worker does the detection, so face-typed events were never
 * actually a requirement — a face-only filter just silently threw away every
 * usable frame.
 *
 * `allowed` is a list of lowercase substrings; the caller owns it so this stays
 * pure and testable (the service reads FACE_CAPTURE_EVENT_TYPES).
 */
export function isCaptureEvent(
  eventType: string | undefined,
  allowed: readonly string[],
): boolean {
  const t = (eventType ?? '').toLowerCase();
  if (!t) return false;
  return allowed.some((a) => a && t.includes(a));
}
