...
export class JrIntakeService {
  ...
  async createExternalMatter(dto: CreateExternalMatterDto, user: RequestUser): Promise<JrMatter> {
    ...
    if (dto.attachToClientId) {
      const client = await this.prisma.client.findFirst({ where: { id: dto.attachToClientId } });
      if (!client) throw new NotFoundException('Client not found.');
      const lead = await this.prisma.lead.findFirst({
        where: { convertedClientId: dto.attachToClientId },
      });
      if (!lead) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const matter = await tx.jrMatter.create({
        data: {
          ...
          deadlineRuleSetVersion: CURRENT_DEADLINE_RULE_SET_VERSION,
          createdByUserId: user.id,
          // authorityDecisionDate is a @db.Date (Date|null) — feed its ISO day to
          // the legal-date normalizer; guard the null.
          decisionCommunicatedAt: kase.authorityDecisionDate
