...
export class OutboundMessageProcessor extends WorkerHost {
  ...
  constructor(
    ...
    // Shared with WebhookIngestProcessor: the common 131053 failure arrives as
    // a STATUS WEBHOOK (send API accepted), so the transcript hook must be
    // callable from both paths — this catch only sees send-API rejections.
    ...
    try {
      ...
      if (err instanceof MetaApiError) {
        ...
        if (!isRetryable) {
          ...
          void this.failedVoice.transcribe(message).catch((e) =>
