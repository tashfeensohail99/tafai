  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <form onSubmit={createRequirement} className="rounded-[28px] border px-4 py-5 shadow-sm sm:px-6" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
          <h3 className="mb-3 text-lg font-semibold" style={{ color: 'var(--color-text-primary)' }}>Create Document Requirement</h3>
          <div className="grid gap-3 md:grid-cols-2">
            <label className="flex flex-col gap-1 text-sm font-medium" style={{ color: 'var(--color-text-secondary)' }}>
              Service Type
              <input value={requirementForm.serviceType} onChange={(event) => setRequirementForm((current) => ({ ...current, serviceType: event.target.value }))} className="rounded-md border px-3 py-2" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }} />
            </label>
            <label className="flex flex-col gap-1 text-sm font-medium" style={{ color: 'var(--color-text-secondary)' }}>
              Target Country
              <input value={requirementForm.targetCountry} onChange={(event) => setRequirementForm((current) => ({ ...current, targetCountry: event.target.value }))} className="rounded-md border px-3 py-2" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }} />
            </label>
            <label className="flex flex-col gap-1 text-sm font-medium md:col-span-2" style={{ color: 'var(--color-text-secondary)' }}>
              Requirement Name
              <input value={requirementForm.name} onChange={(event) => setRequirementForm((current) => ({ ...current, name: event.target.value }))} className="rounded-md border px-3 py-2" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }} />
            </label>
            ...
            <label className="flex flex-col gap-1 text-sm font-medium md:col-span-2" style={{ color: 'var(--color-text-secondary)' }}>
              Description
              <textarea value={requirementForm.description} onChange={(event) => setRequirementForm((current) => ({ ...current, description: event.target.value }))} className="min-h-[96px] rounded-md border px-3 py-2" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }} />
            </label>
          </div>
        ...
        <form onSubmit={uploadDocument} className="rounded-[28px] border px-4 py-5 shadow-sm sm:px-6" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
          <h3 className="mb-3 text-lg font-semibold" style={{ color: 'var(--color-text-primary)' }}>Upload Client Document</h3>
          <div className="grid gap-3 md:grid-cols-2">
            <label className="flex flex-col gap-1 text-sm font-medium" style={{ color: 'var(--color-text-secondary)' }}>
              Client
              <select value={documentForm.clientId} onChange={(event) => setDocumentForm((current) => ({ ...current, clientId: event.target.value }))} className="rounded-md border px-3 py-2" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
            ...
            <label className="flex flex-col gap-1 text-sm font-medium" style={{ color: 'var(--color-text-secondary)' }}>
              Case
              <select value={documentForm.caseId} onChange={(event) => setDocumentForm((current) => ({ ...current, caseId: event.target.value }))} className="rounded-md border px-3 py-2" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
            ...
            <label className="flex flex-col gap-1 text-sm font-medium md:col-span-2" style={{ color: 'var(--color-text-secondary)' }}>
              Requirement
              <select value={documentForm.documentRequirementId} onChange={(event) => setDocumentForm((current) => ({ ...current, documentRequirementId: event.target.value }))} className="rounded-md border px-3 py-2" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
            ...
            <label className="flex flex-col gap-1 text-sm font-medium md:col-span-2" style={{ color: 'var(--color-text-secondary)' }}>
              Document Name
              <input value={documentForm.name} onChange={(event) => setDocumentForm((current) => ({ ...current, name: event.target.value }))} className="rounded-md border px-3 py-2" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }} />
            </label>
            <label className="flex flex-col gap-1 text-sm font-medium md:col-span-2" style={{ color: 'var(--color-text-secondary)' }}>
              File Key
              <input value={documentForm.fileKey} onChange={(event) => setDocumentForm((current) => ({ ...current, fileKey: event.target.value }))} className="rounded-md border px-3 py-2" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }} />
            </label>
            <label className="flex flex-col gap-1 text-sm font-medium md:col-span-2" style={{ color: 'var(--color-text-secondary)' }}>
              Description
              <textarea value={documentForm.description} onChange={(event) => setDocumentForm((current) => ({ ...current, description: event.target.value }))} className="min-h-[96px] rounded-md border px-3 py-2" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }} />
            </label>
          </div>
          <div className="mt-6 flex justify-end">
            <button type="submit" disabled={submitting} className="w-full rounded-md px-4 py-2 text-sm font-medium disabled:opacity-60 sm:w-auto" style={{ backgroundColor: 'var(--color-primary-600)', color: 'var(--color-text-inverse)' }}>
              ...
              ...documentColumns,
              {
                ...
                render: (row: DocumentRow) => (
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => void reviewDocument(row.id, 'VERIFIED')} className="rounded-md border px-3 py-1 text-xs font-medium" style={{ borderColor: 'var(--color-status-success)', color: 'var(--color-status-success)' }}>Verify</button>
                    <button onClick={() => void reviewDocument(row.id, 'REJECTED')} className="rounded-md border px-3 py-1 text-xs font-medium" style={{ borderColor: 'var(--color-status-danger)', color: 'var(--color-status-danger)' }}>Reject</button>
                    <button onClick={() => void reviewDocument(row.id, 'REPLACEMENT_REQUIRED')} className="rounded-md border px-3 py-1 text-xs font-medium" style={{ borderColor: 'var(--color-status-warning)', color: 'var(--color-status-warning)' }}>Replacement</button>
                  </div>
                ),
              },
            ]}
