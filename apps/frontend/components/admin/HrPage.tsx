...
import { PageHeader, GlassCard, PrimaryButton, SecondaryButton, StatusBadge, Field } from '@/components/sales-v2/ui';
  ...
  return (
    <div className="sos-stack" style={{ gap: 20 }}>
      ...
      <GlassCard>
        ...
        {err ? (
          <ErrorState message={err} onRetry={() => load(search.trim() || undefined)} />
        ) : !rows ? (
          <LoadingState />
        ) : rows.length === 0 ? (
          <div style={{ padding: 24, textAlign: 'center', opacity: 0.6 }}>No employees found.</div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table className="sos-table" style={{ width: '100%', minWidth: 780 }}>
              <thead>
                <tr>
                  <th>Name</th><th>Code</th><th>Business email</th><th>Dept</th>
                  <th>Branch</th><th>Designation</th><th>Ext</th><th>Status</th><th></th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r) => (
                    ...
                    <td>
                      {can('hr.offboard') && r.isActive ? (
                        <button
                          ...
                          style={{ color: 'var(--sos-danger, #e5484d)' }}
                        >
                          <UserMinus size={16} />
                        </button>
                      ) : null}
...
function CredentialCard({ result, onDone }: { result: OnboardResult; onDone: () => void }) {
  ...
  const Row = ({ label, value }: { label: string; value: string }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0' }}>
      ...
      <button className="sos-icon-btn" title="Copy" onClick={() => copy(label, value)}>
