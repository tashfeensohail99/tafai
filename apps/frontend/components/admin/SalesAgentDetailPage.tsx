  ...
  const load = useCallback(async () => {
    ...
    try {
      const [emp, leadList, awaitingIds] = await Promise.all([
        ...
        // limit=10000 (the API's max) so the status tabs + KPI counts reflect
        // the agent's FULL roster — without it the endpoint defaults to 250 rows
        // and a busy rep's counts (e.g. "New") would be computed from a slice.
        apiFetch<AssignedLead[]>(`/leads?assignedEmployeeId=${employeeId}&limit=10000`),
