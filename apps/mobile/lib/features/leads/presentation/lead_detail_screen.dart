...
class _LeadDetailScreenState extends ConsumerState<LeadDetailScreen> {
  ...
  /// ([leadWhatsappModeProvider]). `personal` (a TEMPORARY campaign switch,
  /// flipped server-side with no app update) opens the rep's own WhatsApp with
  /// the lead's number; `crm` (default) opens the in-app CRM conversation.
  Future<void> _whatsappButton(Lead lead) async {
    var mode = LeadWhatsappMode.crm;
    try {
      mode = await ref.read(leadWhatsappModeProvider.future);
    } catch (_) {
